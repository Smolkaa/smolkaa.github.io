...
While this approach appears to be quite simple, it greatly depends on the difficulty to derive the CDF's inverse $F^{-1}$. For example, let the PDF of a distribution be given by 
\begin{equation}
    f(x) = \frac{1}{2}\cos x \quad\text{with}\quad x\in\left[-\pi / 2, \pi / 2 \right]
\end{equation}
...
\end{equation}
We can now draw a random uniform number $r_u$, set this equal to $F(x)$, and solve for $x$ to obtain the inverse CDF $F^{-1}(r_u)$:
\begin{align}
