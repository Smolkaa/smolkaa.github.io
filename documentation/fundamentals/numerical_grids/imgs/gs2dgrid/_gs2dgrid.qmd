::: {.column-page-right}
::: {.panel-tabset}

# {{< bi image >}} Figure

::: {#fig-gs2dgrid}
![](/documentation/fundamentals/numerical_grids/imgs/gs2dgrid/gs2dgrid.png){width="1200px"}

Global, structured, two-dimensional numerical grid in cartesian (lower right) and spherical (center and upper left) coordinates in arbitrary units. Arrows indicate the projection from the spherical to the cartesian coordinate system and vice versa. Note that the grid is only two-dimensional in the spherical coordinate system, assuming a constant third-dimension, i.e., radius $\sphericalCoordinateRadius$.
:::

# {{< bi code >}} Julia
```julia
using .ExESS
using CairoMakie, GeoMakie

#::. functions
function gs2dgrid_2d(N_theta=20, N_phi=10;)
    grid = GlobalStructured2DGrid(Float32,1,N_theta,N_phi)
    Xs = vcat(vec.(coords(grid))'...)

    # setup figure
    fig = Figure(;resolution=(600,300))
    ax = Axis(fig[1,1];
        xautolimitmargin=(0,0),
        xlabel="Azimuth",
        yautolimitmargin=(0,0),
        ylabel="Elevation",)
    hidedecorations!(ax; label=false)

    # grid points
    scatter!(ax, rad2deg.(Xs[:,2]), rad2deg.(Xs[:,3]), markersize=3, color=:black)

    # custom grid lines
    for phi in unique(rad2deg.(Xs[:,3]))
        if phi < -85; continue; end
        lines!(ax, [-180, 180], [phi, phi] .- 90/N_phi; color=(:black, 0.3), linewidth=0.5)
    end
    for theta in unique(rad2deg.(Xs[:,2]))
        if theta < -170; continue; end
        lines!(ax, ones(100)*theta .- 180/N_theta, range(-90,90,100); color=(:black, 0.3), linewidth=0.5)
    end

    # save figure
    save(joinpath(@__DIR__, "gs2dgrid_2d.pdf"), fig)
    save(joinpath(@__DIR__, "gs2dgrid_2d.svg"), fig)
    save(joinpath(@__DIR__, "gs2dgrid_2d.png"), fig, px_per_unit=4)
    nothing
end

function gs2dgrid_proj(N_theta=20, N_phi=10;)
    grid = GlobalStructured2DGrid(Float32,1,N_theta,N_phi)
    Xs = vcat(vec.(coords(grid))'...)

    # setup figure
    fig = Figure(;resolution=(600,300))
    ax = MyLTGeoAxis(fig[1,1]; 
        xlabel="Azimuth",
        xticks=[0],
        ylabel="Elevation",) 
    hidedecorations!(ax; label=false);

    # grid points
    scatter!(ax, rad2deg.(Xs[:,2]), rad2deg.(Xs[:,3]), markersize=3, color=:black)

    # custom grid lines
    for phi in unique(rad2deg.(Xs[:,3]))
        if phi < -85; continue; end
        lines!(ax, [-180, 180], [phi, phi] .- 90/N_phi; color=(:black, 0.3), linewidth=0.5)
    end
    for theta in unique(rad2deg.(Xs[:,2]))
        if theta < -170; continue; end
        lines!(ax, ones(100)*theta .- 180/N_theta, range(-90,90,100); color=(:black, 0.3), linewidth=0.5)
    end

    # save figure
    save(joinpath(@__DIR__, "gs2dgrid_proj.pdf"), fig)
    save(joinpath(@__DIR__, "gs2dgrid_proj.svg"), fig)
    save(joinpath(@__DIR__, "gs2dgrid_proj.png"), fig, px_per_unit=4)
    nothing
end

function gs2dgrid_3d(N_theta=20, N_phi=10;)
    grid = GlobalStructured2DGrid(Float32,1,N_theta,N_phi)
    Xs = vcat(vec.(coords(grid))'...)
    Xc = vcat(vec.(coords(GlobalCartesianPosition, grid))'...)

    #
    fig = Figure(;resolution=(400,400))
    ax = Axis3(fig[1,1];
        aspect=:data,
        azimuth=-0.3pi,
        elevation=0.4,
        xlabeloffset=0,
        ylabeloffset=0,
        zlabeloffset=0,)
    hidedecorations!(ax; label=false)

    # filter
    x, y, z = Float64[], Float64[], Float64[]
    for i in eachindex(Xc[:,1])
        if Xc[i,1] < 0; continue; end
        push!(x, Xc[i,1])
        push!(y, Xc[i,2])
        push!(z, Xc[i,3])
    end
    scatter!(ax, x, y, z, markersize=4, color=:black)

    # custom grid lines
    for phi in unique(rad2deg.(Xs[:,3]))
        phi -= 90/N_phi
        x = [cosd(phi) * cos(x) for x in range(-pi/2,pi/2,100)]
        y = [cosd(phi) * sin(x) for x in range(-pi/2,pi/2,100)]
        z = [sind(phi) * ones(100)...]
        lines!(ax, x, y, z; color=(:black, 0.3), linewidth=0.5)
    end
    for theta in unique(rad2deg.(Xs[:,2]))
        if abs(theta) >= 90; continue; end
        theta -= 180/N_theta
        x = [cosd(theta) * cos(x) for x in range(-pi/2,pi/2,100)]
        y = [sind(theta) * cos(x) for x in range(-pi/2,pi/2,100)]
        z = [sin(x) for x in range(-pi/2,pi/2,100)]
        lines!(ax, x, y, z; color=(:black, 0.3), linewidth=0.5)
    end

    # save figure
    save(joinpath(@__DIR__, "gs2dgrid_3d.pdf"), fig)
    save(joinpath(@__DIR__, "gs2dgrid_3d.svg"), fig)
    save(joinpath(@__DIR__, "gs2dgrid_3d.png"), fig, px_per_unit=4)

    nothing
end

#::. call all functions
function run_all()
    gs2dgrid_2d(20,10)
    gs2dgrid_proj(20,10)
    gs2dgrid_3d(20,10)
end
```

# {{< bi code >}} LaTeX

```latex
\begin{tikzpicture}

    \pgfdeclarelayer{background layer}
    \pgfdeclarelayer{foreground layer}
    \pgfsetlayers{background layer, main, foreground layer}

    \node (2d) at (0pt,0pt) {\includegraphics[width=0.5\textwidth]{./gs2dgrid_2d.pdf}};
    \node[below right = 0 and -3.7 of 2d] (proj) {\includegraphics[width=0.5\textwidth]{./gs2dgrid_proj.pdf}};
    
    \begin{pgfonlayer}{background layer}
        \node[below right = -1.4 and -1.5 of proj] (3d) {\includegraphics[width=0.4\textwidth, trim={0 30pt 0 0}, clip]{./gs2dgrid_3d.pdf}};
    \end{pgfonlayer}

    \draw[-latex, very thick] (3d.70) to [in=0, out=90] node[near end, xshift=0, yshift=20]{\footnotesize{spherical projection}} (2d.15);
    
    \draw[-latex, very thick] (2d.220) to [in=180, out=270] node[near end, xshift=20, yshift=-25]{\footnotesize{cartesian projection}} (3d.190);

\end{tikzpicture}
```

Please ensure that the used `.pdf` files are at the correct location.


# {{< bi download >}} Downloads

* {{< downloadthis numerical_grids/imgs/gs2dgrid/gs2dgrid.pdf label="figure.pdf" type="light" dname="figure" >}} {{< downloadthis numerical_grids/imgs/gs2dgrid/gs2dgrid.png label="figure.sng" type="light" dname="figure" >}} 

* {{< downloadthis numerical_grids/imgs/gs2dgrid/gs2dgrid_2d.svg label="2d.svg" type="light" dname="2d" >}} {{< downloadthis numerical_grids/imgs/gs2dgrid/gs2dgrid_2d.pdf label="2d.pdf" type="light" dname="2d" >}} {{< downloadthis numerical_grids/imgs/gs2dgrid/gs2dgrid_2d.png label="2d.png" type="light" dname="2d" >}} 

* {{< downloadthis numerical_grids/imgs/gs2dgrid/gs2dgrid_proj.svg label="proj.svg" type="light" dname="proj" >}} {{< downloadthis numerical_grids/imgs/gs2dgrid/gs2dgrid_proj.pdf label="proj.pdf" type="light" dname="proj" >}} {{< downloadthis numerical_grids/imgs/gs2dgrid/gs2dgrid_proj.png label="proj.png" type="light" dname="proj" >}} 

* {{< downloadthis numerical_grids/imgs/gs2dgrid/gs2dgrid_3d.svg label="3d.svg" type="light" dname="3d" >}} {{< downloadthis numerical_grids/imgs/gs2dgrid/gs2dgrid_3d.pdf label="3d.pdf" type="light" dname="3d" >}} {{< downloadthis numerical_grids/imgs/gs2dgrid/gs2dgrid_3d.png label="3d.png" type="light" dname="3d" >}}

:::
:::