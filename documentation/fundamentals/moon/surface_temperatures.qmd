---
title: "2.6.3 Surface Temperatures"
---
...
{{< include imgs/lunar_surface_temperatures/_lunar_surface_temperatures.qmd >}}


@Butler1997 proposed an analytical surface temperature distribution based on the solar zenith angle $\zenith_s$ with regard to the local surface normal, assuming a perfectly spherical Moon, see @fig-lunar_surface_temperatures. Given the subsolar position $\left(\longitude_s, \latitude_s\right)$, longitude and latitude respectively, the cosine of the zenith angle can be calculated as $\cos\zenith_s = \cos\longitude_s \cdot \cos\latitude_s$. All measurements of the lunar surface temperatures on the dayside suggest a $\cos^{1/4}$ shape with regard to the solar zenith angle [@Vasavada2012; @Hurley2015], though such models break down on the nightside. By introducing a cutoff at the terminators alongside a defined nightside temperature and shifting the distribution up, @Butler1997 arrived at
\begin{equation}
    \temperature (\zenith_s) = \begin{cases}
        \SI{250}{\kelvin} \cdot \cos^{1/4} (\zenith_s) + \SI{100}{\kelvin}, \qquad & \text{ if } \zenith_s \leq \SI{90}{\degree}, \\
        \SI{100}{\kelvin}, & \text{ if } \zenith_s > \SI{90}{\degree}.
...
\begin{equation}
    \temperature (\longitude_s, \latitude_s) = \begin{cases}
        \SI{262}{\kelvin} \cdot \cos^{1/2} \left( \zenith_s \right) + \SI{130}{\kelvin}, \qquad & \text{ if } \left|\longitude_s\right| < \SI{90}{\degree}, \\
        \left( \sum_{i=0}^5 a_i \longitude_s^i \right) + \SI{35}{\kelvin} \left( \sin \overline{\latitude_s} - 1\right), \qquad & \text{ else.} 
    \end{cases} \label{eq:lunar_surface_temperatures_HURLEY} 
\end{equation}
with the longitude $\longitude_s$ and the subsolar co-latitude $\overline{\latitude_s} = \pi/2 - \latitude_s$. The coefficients of the polynomial fit are $a = \left[444.738,\; -448.937,\; 239.668,\; -63.8844,\; 8.34064,\; -0.423502 \right]$ in kelvin. Note that the longitude has to be used in radians and must be positively defined on the range of $\longitude_s \in \left(\pi/2, 3\pi/2\right)$ for the nightside.



{{< include imgs/lunar_surface_temperatures_comparison/_lunar_surface_temperatures_comparison.qmd >}}
...
* Function: [`lunar_surface_temperatures_BUTLER1997`](/api/surfaces/temperature.qmd). Calculates the lunar surface temperatures using the analytical model by @Butler1997. See Eq. \eqref{eq:lunar_surface_temperatures_BUTLER}.
* Function: [`lunar_surface_temperatures_HURLEY2015`](/api/surfaces/temperature.qmd). Calculates the lunar surface temperatures using the analytical model by @Hurley2015. See Eq. \eqref{eq:lunar_surface_temperatures_HURLEY}.