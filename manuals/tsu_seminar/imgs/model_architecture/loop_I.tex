...
\begin{tikzpicture}[
    ...
    \draw[thick, fill=white!95!black, rounded corners=4pt] (-3,2) rectangle (6.75,-10.5);
    \node[anchor=north east, text=TUMBlueDark] at (6.75,2) {\n{\textbf{Loop I} \\ Exospheric Particle Transport \\ and Surface Interactions}};
    ...
    %::. main connections
    \draw[latex-, thick] (LAUNCH.70) -- +(0,2) node[at end, above]{New particles};

    \draw[-latex, thick] (LAUNCH.270) -- (TRAJECTORY.90);
    \draw[-latex, thick] (TRAJECTORY.270) -- (LAND.90);
    \draw[-latex, thick] (LAND.270) -- (SURF.90);
    \draw[-latex, thick] (SURF.270) |- +(-2,-0.5) -- node[rotate=90, left, above] {thermal desorption; photodesorption; sputtering} ($(LAUNCH.90)+(-2,0.5)$) -| (LAUNCH.110);
